...
//			User owner = propertyUuidMap.get(propertyId).getOwners().get(0).toCommonUser();
			User owner = connection.getConnectionHolders().get(0).toCommonUser();
			if (!CollectionUtils.isEmpty(connection.getConnectionHolders())) {
				owner = connection.getConnectionHolders().get(0).toCommonUser();
			}
...
//			Long expiryDate = (Long) financialYearMaster.get(WSCalculationConstant.Demand_Expiry_Date_String);
			Long expiryDate = demandService.getBillExpiryDate(requestInfo, tenantId);
